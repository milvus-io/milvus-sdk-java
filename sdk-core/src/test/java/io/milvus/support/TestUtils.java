...
public class TestUtils {
    ...
    public static File dockerComposeFile(String fileName) {
        File modulePath = new File("src/test/resources/docker", fileName);
        if (modulePath.isFile()) {
            return modulePath;
        }
        return new File("sdk-core/src/test/resources/docker", fileName);
    }

    public static synchronized void startMilvusStandalone(File composeFile, File volumeDirectory, List<String> containerNames) {
        ...
        // Idempotent shared lifecycle: the same JVM runs the split system-test
        // classes sequentially, so start the compose stack once per compose file
        // and tear it down once at JVM exit (see the shutdown hook below).
        File absCompose = composeFile.getAbsoluteFile();
        if (startedMilvusStandalone) {
            if (startedDockerComposeFile != null && startedDockerComposeFile.equals(absCompose)) {
                return;
            }
            stopMilvusStandalone();
        }
        ...
        try {
            ...
            startedDockerComposeFile = absCompose;
    ...
    private static synchronized void registerShutdownHook() {
        ...
        Runtime.getRuntime().addShutdownHook(new Thread(TestUtils::stopMilvusStandalone));
    }

...
class ParamUtilsTest {
    ...
    @Test
    void legacySearchConversionUsesTimestampAcrossEndpoints() {
        ...
        SearchRequest legacyRequest = ParamUtils.convertSearchParam(searchParam);
        SearchRequest endpointRequest = ParamUtils.convertSearchParam(searchParam, "host-a:19530");
        ...
        SearchParam defaultConsistencyParam = SearchParam.newBuilder()
                ...
                .withFloatVectors(Collections.singletonList(Arrays.asList(0.1F, 0.2F)))
                .build();
        SearchRequest defaultConsistencyRequest = ParamUtils.convertSearchParam(defaultConsistencyParam);

        assertEquals(200L, legacyRequest.getGuaranteeTimestamp());
        assertEquals(100L, endpointRequest.getGuaranteeTimestamp());
        assertEquals(200L, defaultConsistencyRequest.getGuaranteeTimestamp());
    }
}

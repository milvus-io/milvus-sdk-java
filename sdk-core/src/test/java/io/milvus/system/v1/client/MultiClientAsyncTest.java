...
class MultiClientAsyncTest extends MilvusMultiDockerTestBase {
    ...
    void testAsyncMethods() {
        ...
        // wait until all inserted rows are visible after flush
        long expectedRowCount = 10L * rowCount;
        long deadline = System.currentTimeMillis() + TimeUnit.SECONDS.toMillis(60);
        while (stat.getRowCount() < expectedRowCount && System.currentTimeMillis() < deadline) {
            try {
                Thread.sleep(1000L);
            ...
            R<GetCollectionStatisticsResponse> retryStat = client.getCollectionStatistics(
                    GetCollectionStatisticsParam.newBuilder()
                            .withCollectionName(randomCollectionName)
                            .withFlush(true)
                            .build());
            assertEquals(R.Status.Success.getCode(), retryStat.getStatus().intValue());
            stat = new GetCollStatResponseWrapper(retryStat.getData());
            System.out.println("Collection row count after wait: " + stat.getRowCount());
        }
        assertEquals(expectedRowCount, stat.getRowCount());
        assertTrue("collection rows not fully visible after flush: expected=" + expectedRowCount
                + " actual=" + stat.getRowCount(), stat.getRowCount() == expectedRowCount);
        ...
        // query async
        String expr = field1Name + " in " + distinctQueryIDs;
